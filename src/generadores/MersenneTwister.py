...
class MT19937:
    ...
    def initialize(self, seed):
        ...
        for i in range(1, MT19937.n):
            self.X[i] = (MT19937.f * (self.X[i - 1] ^ (self.X[i - 1]
                         >> (MT19937.w - 2))) + i) & ((1 << MT19937.w) - 1)
        self.twist()
    ...
    def twist(self):
        for i in range(MT19937.n):
            ...
            tmp = (self.X[i] & upper_mask) + \
                (self.X[(i + 1) % MT19937.n] & lower_mask)
            tmpA = tmp >> 1
            if (tmp % 2):
                tmpA = tmpA ^ MT19937.a
    ...
    def random(self):
        return self.temper() / float(1 << MT19937.w)
